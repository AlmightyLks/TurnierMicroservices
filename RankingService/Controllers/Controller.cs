...
namespace RankingService.Controllers
{
    public class Controller
    {
        ...
        {
            Mannschaften = FetchMannschaften<List<Mannschaft>>(Microservices.MannschaftsServiceApi) ?? Mannschaften;
        }
        ...
        private T FetchMannschaften<T>(string target)
        {
            T result = default(T);
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    HttpResponseMessage response = client.GetAsync($"{target}").GetAwaiter().GetResult();
                    string jsonStr = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                    string jsonStrr = JsonConvert.DeserializeObject<string>(jsonStr, new JsonSerializerSettings
                    {
                        TypeNameHandling = TypeNameHandling.All
                    });
                    result = JsonConvert.DeserializeObject<T>(jsonStrr, new JsonSerializerSettings
                    {
                        TypeNameHandling = TypeNameHandling.All
                    });
                }
